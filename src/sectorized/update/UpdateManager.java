...
public class UpdateManager implements Manager {
    ...
    public void init() {
        Events.run(EventType.Trigger.update, () -> {
            ...
            if (interval.get(1, Config.c.interval.infoMessage)) {
            ...
            if (interval.get(3, Config.c.interval.gameStateLog)) {
            ...
            if (interval.get(2, Config.c.interval.dominationCheck)) {
                ...
                if (!lock) {
                    ...
                    if (healthPerTeamEntries.size() > 1 && healthPerTeamEntries.get(0)
                            .getValue() >= healthPerTeamEntries.get(1).getValue() + Config.c.game.domination.healthThreshold + Config.c.game.domination.healthPerWave * state.wave) {
        ...
        Events.on(SectorizedEvents.BiomesGeneratedEvent.class, event -> {
            coreDominationDifference = State.planet.equals(Planets.serpulo.name) ? Config.c.game.domination.coreLead.serpulo : Config.c.game.domination.coreLead.erekir;
        });
        ...
        Events.on(EventType.WaveEvent.class, event -> {
            ...
            state.rules.blockDamageMultiplier = (float) (Config.c.multiplier.blockDamageFormulaA + (Config.c.multiplier.blockDamageFormulaC / (Math.pow(state.wave * Config.c.multiplier.blockDamageFormulaB, 4) + 1)));
            state.rules.unitDamageMultiplier = (float) (Config.c.multiplier.unitDamageFormulaA - (Config.c.multiplier.unitDamageFormulaB / (Math.pow(state.wave * Config.c.multiplier.unitDamageFormulaC, 4) + 1)));
